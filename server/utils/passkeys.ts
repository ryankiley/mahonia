...
/** The guard every passkey ceremony endpoint opens with: refuse up front (503 +
 *  one loud server log) on a deploy with no shared challenge store, rather than
 *  failing at verify with nothing to explain it. One place, so the four ceremony
 *  routes can't drift on the status or the log line. */
export function requirePasskeys(): void {
