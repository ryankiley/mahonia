    ...
    <main id="main-content" tabindex="-1" class="wrap page vault__page">
      ...
      <ClientOnly>
        ...
        <!-- Signed out. The subtitle above already says what a vault IS, so this
             ...
             WORD FOR WORD the side panel's prompt (VaultBrowser). It is the same
...
/* balance, as the panel's copy of this sentence takes: it's centred, so a ragged
   right edge reads as a mistake rather than as rag, and at a narrow window the last
   word would otherwise strand on its own line. */
.vault__sentline {
  color: var(--ink);
  text-wrap: balance;
}
