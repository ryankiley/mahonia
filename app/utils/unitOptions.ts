 ...
 *
 * Four components were each mapping the same constants into the same `{ key, label }`
 * shape — TotalsBar, ListHead, TrailPlanPanel, and the editor once it grew a headline of
 * its own. Four copies of one line is not expensive, but it is four places a label could
 * start differing from its neighbours, and the whole point of these pickers is that the
 * same gesture reads the same wherever it appears.
 *
