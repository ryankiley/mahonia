...
describe("gear cost", () => {
  ...
  const only = async () => (await listVaultItems(db as any, VAULT))[0]!;

  it("records a price and reads it back", async () => {
    const id = (await only()).id;
    expect(await setVaultItemPrice(db as any, VAULT, id, 59_900)).toBe(true);
    expect((await only()).priceCents).toBe(59_900);
  });

  it("clears with null", async () => {
    const id = (await only()).id;
    await setVaultItemPrice(db as any, VAULT, id, 59_900);
    await setVaultItemPrice(db as any, VAULT, id, null);
    expect((await only()).priceCents).toBeUndefined();
  });

  it("clamps a negative to zero and an absurd figure to the cap", async () => {
    const id = (await only()).id;
    await setVaultItemPrice(db as any, VAULT, id, -500);
    expect((await only()).priceCents).toBe(0);
    await setVaultItemPrice(db as any, VAULT, id, 99_999_999_999);
    expect((await only()).priceCents).toBe(100_000_000);
  });

  it("won't let one account price another's gear", async () => {
    const id = (await only()).id;
    expect(await setVaultItemPrice(db as any, 2, id, 1000)).toBe(false);
    expect((await only()).priceCents).toBeUndefined();
  });

  it("stamps the currency with the figure, and clears it with the figure", async () => {
    const id = (await only()).id;
    await setVaultItemPrice(db as any, VAULT, id, 59_900, "GBP");
    expect((await only()).currency).toBe("GBP");
    await setVaultItemPrice(db as any, VAULT, id, null, "GBP");
    expect((await only()).currency).toBeUndefined();
  });

  it("survives a later capture — a price is yours, and lists rarely carry one", async () => {
    const id = (await only()).id;
    await setVaultItemPrice(db as any, VAULT, id, 59_900);
    await captureVaultItems(db as any, VAULT, [cap]); // no price on the capture
    expect((await only()).priceCents).toBe(59_900);
  });
});
