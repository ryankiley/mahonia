...
describe("wornQty (the worn split on a base line)", () => {
  ...
  it("drops a folder's legacy sortBy on add and on patch — folders are drag order only", () => {
    const s = base();
    applyOps(s, [
      { t: "addFolder", folder: { id: "fa", name: "A", defaultClassification: "base", sortBy: "name", sortOrder: 2 } as any },
    ]);
    expect("sortBy" in s.folders.find((f) => f.id === "fa")!).toBe(false);
    applyOps(s, [{ t: "updateFolder", id: "f1", patch: { sortBy: "heaviest" } as any }]);
    expect("sortBy" in s.folders.find((f) => f.id === "f1")!).toBe(false);
  });
});
